...
diff = np.load("swap/dop_diff.npy", allow_pickle=True)
dop =np.load("swap/dop.npy", allow_pickle=True)
...
plt.title("$\mathcal{T} = 5000$")
plt.xlabel("Proportion of B particles p")
plt.ylabel("Normalized measurements") 

fig_1 ,= plt.plot(T, diff)
fig_2 ,= plt.plot(T, dop)
fig_3 ,= plt.plot(T, surv_prob)

plt.legend(handles=[fig_1,fig_2,fig_3],labels=['$\partial \mathcal{D}_d / \partial p$', '$\mathcal{D}_d$', 'Survivial Prob'],loc='best')
